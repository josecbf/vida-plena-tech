/**
 * CLI — Importador Prover, Fase 6B.2, APPLY de INSCRIÇÕES de Ensino.
 *
 *   pnpm prover:teaching-registrations:apply --file ./data/export.zip --limit 100 --confirm APPLY
 *   pnpm prover:teaching-registrations:apply --file ./data/export.zip --confirm APPLY   (FULL)
 *
 * Cria TeachingRegistration. Exige --confirm APPLY. Pagamento/lote PRESERVADOS em
 * metadata (sem financeiro). NÃO cria presença, NÃO altera Teaching/Person/status/User/Role.
 */
import { PrismaClient } from "@prisma/client";
import { loadProverTeachingRegistrations } from "../src/modules/integrations/prover/zip";
...
async function main() {
  ...
  if (args.confirm !== "APPLY") {
    console.error(
      "\n✖ APPLY recusado. Este comando ESCREVE no banco (cria inscrições de ensino).\n" +
        "  Use --confirm APPLY. Ex.: pnpm prover:teaching-registrations:apply --file ./data/export.zip --limit 100 --confirm APPLY\n" +
        "  (Para só analisar, use: pnpm prover:teaching:dry-run --file <...>)\n",
    );
    process.exit(2);
  }
  if (!args.file) { console.error("Uso: pnpm prover:teaching-registrations:apply --file <export.zip> [--limit N] --confirm APPLY"); process.exit(2); }
  ...
  try {
    ...
    console.log(`\n▶ APPLY de INSCRIÇÕES de Ensino — lendo: ${args.file}`);
    ...
    console.log(`  ⚠ MODO APPLY — cria TeachingRegistration; pagamento/lote só em metadata; NÃO cria presença.\n`);

    const t = tenant.id;
    const cnt = async () => ({
      reg: await prisma.teachingRegistration.count({ where: { tenantId: t } }),
      map: await prisma.externalMapping.count({ where: { tenantId: t, system: "PROVER", externalType: "teaching_registration" } }),
      ...
      member: await prisma.person.count({ where: { tenantId: t, status: "MEMBER" } }),
      user: await prisma.user.count(), role: await prisma.roleAssignment.count(),
    });
    const before = await cnt();

    const r = await runTeachingRegistrationsApply(prisma, { tenantId: t, fileName: "inscrições de ensino", registrations, limit: args.limit });
    ...
    console.log("  APPLY — Inscrições de Ensino (Fase 6B.2)");
    console.log("═".repeat(64));
    ...
    console.log(line("Ensino resolvido / não resolvido", `${r.teachingResolved} / ${r.teachingNotFound}`));
    ...
    console.log(line("Status encontrados", JSON.stringify(r.statusCounts)));
    console.log(line("Nota numérica (grade) / texto preservado", `${r.gradeNumeric} / ${r.gradePreserved}`));
    console.log(line("Pagamento/lote detectado / preservado (metadata)", `${r.paymentDetected} / ${r.paymentPreserved}`));
    console.log("═".repeat(64));
    console.log(line("TeachingRegistration ANTES → DEPOIS", `${before.reg} → ${after.reg}`));
    console.log(line("ExternalMapping(teaching_registration) A → D", `${before.map} → ${after.map}`));
    console.log(line("Teaching ANTES → DEPOIS", `${before.teaching} → ${after.teaching} ${flag(before.teaching, after.teaching)}`));
    console.log(line("TeachingModule ANTES → DEPOIS", `${before.module} → ${after.module} ${flag(before.module, after.module)}`));
    console.log(line("TeachingLesson ANTES → DEPOIS", `${before.lesson} → ${after.lesson} ${flag(before.lesson, after.lesson)}`));
    console.log(line("TeachingSession ANTES → DEPOIS", `${before.session} → ${after.session} ${flag(before.session, after.session)}`));

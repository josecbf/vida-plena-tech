/**
 * CLI — Importador Prover, Fase 6B.3, APPLY de PRESENÇAS de Ensino.
 *
 *   pnpm prover:teaching-attendance:apply --file ./data/export.zip --limit 200 --confirm APPLY
 *   pnpm prover:teaching-attendance:apply --file ./data/export.zip --confirm APPLY   (FULL)
 *
 * Cria TeachingAttendance. Exige --confirm APPLY. NÃO altera Teaching/Session/
 * Registration/Person/status, NÃO cria User/Role, NÃO cria financeiro.
 */
import { PrismaClient } from "@prisma/client";
import { loadProverTeachingAttendances } from "../src/modules/integrations/prover/zip";
...
async function main() {
  ...
  if (args.confirm !== "APPLY") {
    console.error(
      "\n✖ APPLY recusado. Este comando ESCREVE no banco (cria presenças de ensino).\n" +
        "  Use --confirm APPLY. Ex.: pnpm prover:teaching-attendance:apply --file ./data/export.zip --limit 200 --confirm APPLY\n" +
        "  (Para só analisar, use: pnpm prover:teaching:dry-run --file <...>)\n",
    );
    process.exit(2);
  }
  if (!args.file) { console.error("Uso: pnpm prover:teaching-attendance:apply --file <export.zip> [--limit N] --confirm APPLY"); process.exit(2); }
  ...
  try {
    ...
    console.log(`\n▶ APPLY de PRESENÇAS de Ensino — lendo: ${args.file}`);
    ...
    console.log(`  ⚠ MODO APPLY — cria TeachingAttendance; NÃO altera inscrição/pessoa; NÃO cria financeiro.\n`);

    const t = tenant.id;
    const cnt = async () => ({
      att: await prisma.teachingAttendance.count({ where: { tenantId: t } }),
      map: await prisma.externalMapping.count({ where: { tenantId: t, system: "PROVER", externalType: "teaching_attendance" } }),
    ...
    const r = await runTeachingAttendanceApply(prisma, { tenantId: t, fileName: "presenças de ensino", attendances, limit: args.limit });
    ...
    console.log("  APPLY — Presenças de Ensino (Fase 6B.3)");
    console.log("═".repeat(64));
    console.log(line("Processadas", r.read));
    ...
    console.log(line("Aproveitamento detectado / preservado (score)", `${r.scoreDetected} / ${r.scorePreserved}`));
    console.log("═".repeat(64));
    console.log(line("TeachingAttendance ANTES → DEPOIS", `${before.att} → ${after.att}`));
    console.log(line("ExternalMapping(teaching_attendance) A → D", `${before.map} → ${after.map}`));
    ...
    console.log(line("TeachingRegistration ANTES → DEPOIS", `${before.reg} → ${after.reg} ${flag(before.reg, after.reg)}`));

...
- **Validação externa feita:** GPT + Claude (infra) e Gemini (identidade). Síntese em `docs/Técnico/Validacao de Arquitetura - Consolidacao.md`; decisões em `docs/Técnico/ADRs/` (0001–0006). **Pendente:** rodar infra no Gemini e identidade no GPT/Claude (cobertura 3×2) e fechar os ADRs `Proposto` (0002 TAP, 0005 identidade, 0006 LGPD) + sub-decisões (worker, service-role, PSP Pix).
